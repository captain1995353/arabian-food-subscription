 ...
 * subscriptions. The file goes to the public `receipts` bucket; the URL is
 * saved on the subscription so the admin can verify it on the Payments page.
 */
export async function uploadReceipt(
  ...
  const { data: pub } = service.storage.from("receipts").getPublicUrl(path);
